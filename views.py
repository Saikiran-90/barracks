...
def bookForm(request,room_id):
      if request.method == 'POST': 
        ...
        email = my_user.email
        return render(request,'book.html',{'name':name,'email':email,'rm':room_list,'room_no':room_no})
      else:
            return HttpResponse('404 - Not Found')
...
def updateForm(request,room_id):
    ...
    return render(request,'updateRoom.html',{'room_id':obj.room_id})
...
def itemUpdateForm(request,item_id):
    ...
    return render(request,'updateItem.html',{'item_id':obj.item_id})
